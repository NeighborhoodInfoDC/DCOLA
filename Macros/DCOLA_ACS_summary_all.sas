/**************************************************************************
 Program:  DCOLA_ACS_summary_all.sas
 Library:  DCOLA
 ...
 Modifications: 
	9/22/16 - adopted to DCOLA library;
			  renamed macros with "DCOLA_" prefix.
**************************************************************************/

%macro DCOLA_ACS_summary_all( 
  ...
  %if %mparam_is_yes( &_finalize ) and not &_remote_batch_submit %then %do;
    %warn_mput( macro=DCOLA_ACS_summary_all, msg=%str(Not a remote batch submit session. Finalize will be set to N.) )
    %let _finalize = N;
  %end;
  ...
  **** Create summary files from block group source ****;
  ...
  %DCOLA_ACS_summary_geo_source( bg10 )


  **** Create summary files from census tract source ****;

  %DCOLA_ACS_summary_geo_source( tr10 )


%mend DCOLA_ACS_summary_all;
