/**************************************************************************
 Program:  NCDB_2000_summary_all.sas
 Library:  DCOLA
 ...
 Description:  Main macro for generating all NCBD 2000 summary files.

**************************************************************************/

%macro NCDB_2000_summary_all( 
  ...
  %if %mparam_is_yes( &_finalize ) and not &_remote_batch_submit %then %do;
    %warn_mput( macro=NCDB_2000_summary_all, msg=%str(Not a remote batch submit session. Finalize will be set to N.) )
    %let _finalize = N;
  %end;
  ...
  **** Create summary files from census tract source ****;
  ...
  %NCDB_2000_summary_geo_source( tr00 )


%mend NCDB_2000_summary_all;
