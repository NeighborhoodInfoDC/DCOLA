/*******************************************
* Program: OCCUP_CAT_PUMA_2000.sas
* Library: DCOLA
...
data ipums_OCC_UNWGT2000; 
set IPUMS.ipums_2000_dc   (keep= AGE PERWT HISPAND racgen00 SEX OCC
	where=(AGE >=16));
...
OCC_LARGE  =PUT(OCC,OCC_00_BIG.);
TOTSAMP = 1;

Run;

proc download inlib=work outlib=dcola; 
select ipums_OCC_UNWGT2000;
run;

endrsubmit;	


proc means data=dcola.ipums_OCC_UNWGT2000 chartype;
      var TOTSAMP;
      class  RACE_ETH OCC_LARGE;
	  weight PERWT;
      output out=DCOLA.OCCUP_LTTL_2000 sumwgt=;
run;

filename fexport "D:\DCDATA\Libraries\DCOLA\Raw\OccupCat_Race_2000_PUMA.csv" lrecl=5000;
proc export data=DCOLA.OCCUP_LTTL_2000
	outfile=fexport 
	dbms=csv replace;
	run;
